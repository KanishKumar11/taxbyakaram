...
export default function EditBlogPost({ params }) {
    ...
    const fetchBlogPost = async () => {
      try {
        ...
        setFormData({
          ...
          thumbnail: null, // We don't load the actual file, just the URL
        });
        setOriginalSlug(response.slug);
  ...
  const validateForm = () => {
    ...
    if (!isSlugAvailable && formData.slug !== originalSlug)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Only handle thumbnail if a new one was uploaded
      if (formData.thumbnail) {
        ...
        thumbnailUrl = storage.getFileView(BUCKET_ID, thumbnailResponse.$id);
      }
            ...
            <div className="relative">
              <input
                ...
                className={`w-full px-4 py-2 rounded-lg border focus:outline-none focus:ring-2 ${
                  isSlugAvailable || formData.slug === originalSlug
                    ? "focus:ring-blue-500"
                    : "focus:ring-red-500"
                }`}
